    ...
    if not settings.enabled then -- auto add words
        DictQuickLookUp.tweak_buttons_func = function(obj, buttons)
            ...
            table.insert(buttons[1], #buttons[1], {
                ...
                font_bold = false,
                callback = function()
                    local book_title = obj.ui.doc_settings and obj.ui.doc_settings:readSetting("doc_props").title or
                    _("Dictionary lookup")
        ...
        HorizontalGroup:new {
            ...
            HorizontalGroup:new {
                ...
                VerticalGroup:new {
                    ...
                    LeftContainer:new {
                        dimen = Geom:new { w = text_max_width, h = math.floor(self.height - word_height -
                        self.v_spacer.width * 2.2) },
                        HorizontalGroup:new {
                            subtitle_prefix,
                            TextWidget:new {
                                text = self.item.book_title,
                                face = subtitle_italic_face,
                                max_width = math.ceil(math.max(5,
                                    text_max_width - subtitle_prefix:getSize().w - Size.padding.fullscreen)),
                                fgcolor = subtitle_color
                            }
                        }
                    },
...
function VocabularyBuilderWidget:init()
    ...
    self.review_button_width = math.min(math.max(self.review_button_width, temp_button:getSize().w), Screen:getWidth() /
    4)
    temp_button:free()
...
function VocabularyBuilderWidget:refreshFooter()
    ...
    table.insert(self.page_info, has_sync and self.footer_sync or HorizontalSpan:new { width = self
    .footer_left_corner_width })
    ...
    table.insert(self.page_info,
        has_search and self.footer_search or HorizontalSpan:new { width = self.footer_right_corner_width })
end
...
function VocabularyBuilderWidget:setupItemHeight()
    ...
    self.item_margin = self.item_margin +
    math.floor((content_height - self.items_per_page * line_height) / self.items_per_page)
