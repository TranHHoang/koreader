...
function ReaderDictionary:onLookupWord(word, is_sane, boxes, highlight, link, selected_text)
    -- error("Callstack")
    ...
    if selected_text then
        while true do
            local pos = self.ui.document:getPrevVisibleWordStart(selected_text.pos0)
            if not pos then break end
            
            local text = self.ui.document:getTextFromXPointers(pos, selected_text.pos0)
